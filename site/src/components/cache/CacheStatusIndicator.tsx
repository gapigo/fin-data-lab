...
export function CacheStatusIndicator({ compact = false }: CacheStatusIndicatorProps) {
    ...
    if (loading || !stats) {
        return (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--bg-elevated)]/50 text-[var(--text-muted)] text-sm">
    ...
    if (compact) {
        return (
            ...
            >
                ...
                {expanded && (
                    <div className="absolute top-full right-0 mt-2 p-4 bg-slate-900 rounded-xl border border-[var(--border-default)] shadow-xl z-50 min-w-[250px]">
    ...
    return (
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl border border-[var(--border-default)]/50 overflow-hidden">
            {/* Header */}
            <button
                onClick={() => setExpanded(!expanded)}
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        <h3 className="text-sm font-semibold text-[var(--text-primary)]">Cache de Dados</h3>
            ...
            {/* Details */}
            {expanded && (
                <div className="px-4 pb-4 border-t border-[var(--border-default)]/50">
                    <CacheDetails stats={stats} onClear={handleClear} onRefresh={refreshStats} clearing={clearing} />
                </div>
            )}
        </div>
    ...
    return (
        <div className="pt-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
                <div className="bg-[var(--bg-elevated)]/50 rounded-lg p-3 text-center">
                    <p className="text-2xl font-bold text-emerald-400">{stats.todayEntries}</p>
                    <p className="text-xs text-[var(--text-muted)]">Hoje</p>
                </div>
                <div className="bg-[var(--bg-elevated)]/50 rounded-lg p-3 text-center">
                ...
                <div className="bg-[var(--bg-elevated)]/50 rounded-lg p-3 text-center">
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Tamanho</p>
                </div>
            </div>

            {/* Info Text */}
            <div className="text-xs text-[var(--text-muted)] bg-[var(--bg-elevated)]/30 rounded-lg p-3">
                <p className="flex items-center gap-2 mb-1">
                    <Zap className="w-3 h-3 text-amber-400" />
                    <span className="text-[var(--text-secondary)] font-medium">Como funciona:</span>
                </p>
                <p>
            ...
            <div className="flex gap-2">
                <button
                    onClick={onRefresh}
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-[var(--text-primary)] text-sm transition-colors"
                >
